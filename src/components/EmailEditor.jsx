...
export default function EmailEditor() {
  ...
  const handleGenerateAI = async (index) => {
    ...
    updated[index] = {
      ...
      body: `Hi ${state.data.recipient.name || "there"},\n\nI noticed ${
        state.data.recipient.signalDescription
      }. \n\nAt ${state.data.sender.company}, ${
        state.data.sender.oneLiner
      }. Given your focus on ${
        state.data.recipient.industry
      }, I think there's a strong fit for ${state.data.partnership.type}.\n\n${
        state.data.sender.benefits[0]
      }. ${state.data.sender.socialProof}.\n\nWould you be open to ${
        state.data.partnership.cta
      }?`,
      generatedBy: "ai",
      aiRationale:
        "This email references the recent signal, ties your product to their industry, includes social proof, and has a clear CTA.",
    };
    setEmails(updated);
  ...
  const updateEmail = (field, value) => {
    ...
    updated[currentEmail] = {
      ...
      [field]: value,
      generatedBy:
        field === "aiEnabled" ? updated[currentEmail].generatedBy : "manual",
    };
    setEmails(updated);
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Write or generate your email sequence with AI
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 mb-8">
        <div className="mb-4">
          <h3 className="text-lg font-medium text-gray-900 mb-1">
            Email {emails[currentEmail].stepNumber}:{" "}
            {template.steps[currentEmail].label}
          </h3>
          <p className="text-sm text-gray-600">
            {template.steps[currentEmail].description}
          </p>
        </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    AI will personalize content based on your inputs and the
                    recipient&apos;s signal
                  </p>
                </div>
              </label>
        ...
        {emails[currentEmail].generatedBy === "ai" &&
          emails[currentEmail].aiRationale && (
        ...
        <div className="mb-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="flex flex-wrap gap-2">
            {placeholders.map((ph) => (
              <code
                key={ph}
                className="text-xs bg-white px-2 py-1 rounded border border-gray-300 text-blue-600"
              >
                {ph}
              </code>
            ))}
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <span className="text-gray-500">
                ({emails[currentEmail].subject.length}/60)
              </span>
            </label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Email Body *{" "}
              <span className="text-gray-500">
                ({emails[currentEmail].body.split(" ").filter((w) => w).length}{" "}
                words)
              </span>
            </label>
            <textarea
            ...
            <p className="text-xs text-gray-500 mt-1">
              💡 Tip: Keep it under 150 words for best response rates. You can
