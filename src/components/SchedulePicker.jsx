...
import { addMinutes, addHours, addDays, setHours, setMinutes, formatDate, formatTime, formatDateTime, formatRelativeTime } from '@/utils/dateUtils';
            ...
            {days.map((day, idx) => (
              <button
                ...
                className={`
                  ...
                  ${!day ? 'invisible' : ''}
                  ${isDateDisabled(day) ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-blue-50 cursor-pointer'}
                  ${isSelected(day) ? 'bg-blue-600 text-white hover:bg-blue-700' : ''}
                  ${isToday(day) && !isSelected(day) ? 'bg-blue-100 text-blue-600 font-bold' : ''}
                  ${!isSelected(day) && !isToday(day) && !isDateDisabled(day) ? 'text-gray-700' : ''}
                `}
              >
                {day}
              </button>
...
function CustomTimePicker({ value, onChange, onKeyDown }) {
  ...
  // Handle Esc key when picker is open
  useEffect(() => {
    if (!isOpen) return;
  ...
  const togglePeriod = () => {
    ...
    newDate.setHours(currentHours >= 12 ? currentHours - 12 : currentHours + 12);
    onChange(newDate);
  };
        ...
        <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 p-4 w-80">
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center">Hour</h4>
            ...
            {/* Minutes */}
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center">Minute</h4>
            ...
            <div className="flex flex-col gap-2">
              <h4 className="text-sm font-semibold text-gray-700 mb-2 text-center">Period</h4>
              <button
                type="button"
...
export default function SchedulePicker() {
  ...
  const schedulesKey = schedules.map(s => s.scheduledTime.getTime()).join(',');
  
  useEffect(() => {
    const validated = validateSchedule(schedules, currentTime);
    
    // Only update if validation actually changed
    const needsUpdate = validated.some((v, i) => {
      const current = schedules[i];
      return v.status !== current.status ||
             v.errors?.length !== current.errors?.length ||
             v.warnings?.length !== current.warnings?.length;
    });
    ...
    const canProgress = validated.every(s => s.status !== 'invalid');
    dispatch({ type: 'SET_CAN_PROGRESS', payload: canProgress });
  }, [schedulesKey, currentTime, dispatch]);

  const updateTime = (emailId, newTime) => {
    // Ensure seconds and milliseconds are zero
    const cleanTime = new Date(newTime);
    cleanTime.setSeconds(0);
    cleanTime.setMilliseconds(0);
    
    setSchedules(prev => prev.map(s => 
      s.emailId === emailId ? { ...s, scheduledTime: cleanTime } : s
    ));
  };
  ...
  const adjustTime = (emailId, minutes) => {
    ...
    // Ensure first email is at least 5 minutes from now
  ...
  const presets = [
    { 
      ...
      icon: '🌅',
      getValue: () => {
        const date = setMinutes(setHours(addDays(currentTime, 1), 9), 0);
        date.setSeconds(0);
    ...
    { 
      ...
      icon: '⏱️',
      getValue: () => {
        const date = addHours(currentTime, 2);
        date.setSeconds(0);
        date.setMilliseconds(0);
        return date;
      }
    ...
    { 
      ...
      icon: '📅',
      getValue: () => {
        const date = setMinutes(setHours(addDays(currentTime, 7), 9), 0);
        date.setSeconds(0);
        date.setMilliseconds(0);
        return date;
      }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      {/* Header */}
      <div className="mb-10">
        <h2 className="text-3xl font-bold text-gray-900 mb-3">Schedule Your Sequence</h2>
        <p className="text-lg text-gray-600">Set when each email should be sent</p>
        <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-full">
          ...
          <span className="text-sm font-medium text-gray-900">{formatDateTime(currentTime)}</span>
        </div>
      </div>
              ...
              <div className="px-6 py-5 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-gray-900">Email {schedule.stepNumber}</h3>
                      <p className="text-sm text-gray-600">{getEmailLabel(schedule.stepNumber)}</p>
                    </div>
                  </div>
              ...
              <div className="px-6 py-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                  <div>
                    ...
                    <CustomDatePicker
                      ...
                      onChange={(newDate) => {
                        ...
                        combined.setHours(schedule.scheduledTime.getHours());
                        combined.setMinutes(schedule.scheduledTime.getMinutes());
                        combined.setSeconds(0);
                        combined.setMilliseconds(0);
                        updateTime(schedule.emailId, combined);
                      }}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <CustomTimePicker
                      value={schedule.scheduledTime}
                      onChange={(newTime) => updateTime(schedule.emailId, newTime)}
                  ...
                  <div className="mb-5 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {presets.map(preset => (
                        <button
                          key={preset.label}
                          onClick={() => updateTime(schedule.emailId, preset.getValue())}
                ...
                <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100 mb-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-base font-semibold text-gray-900">{formatDateTime(schedule.scheduledTime)}</p>
                      <p className="text-sm font-bold text-blue-600 mt-1">{formatRelativeTime(schedule.scheduledTime)}</p>
                      {index > 0 && (
                        <p className="text-xs text-gray-600 mt-2">
                          {Math.floor((schedule.scheduledTime - schedules[index-1].scheduledTime) / 86400000)} days after Email {schedule.stepNumber - 1}
                        </p>
                      )}
                    </div>
                  </div>
                ...
                {!hasErrors && index > 0 && (
                  <div className="flex items-center gap-2 text-sm text-gray-600 p-3 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <span>Correctly scheduled after Email {schedule.stepNumber - 1}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-8 mb-8 shadow-sm">
        ...
        <div className="relative py-8">
          <div className="relative flex justify-between items-center">
            {/* Timeline Line */}
            <div className="absolute top-7 left-0 right-0 h-1 bg-gradient-to-r from-blue-200via-indigo-200 to-blue-200 rounded-full" style={{ 
              left: schedules.length > 1 ? `${(100 / schedules.length / 2)}%` : '50%',
              right: schedules.length > 1 ? `${(100 / schedules.length / 2)}%` : '50%'
            }} />
            
            {/* Timeline Points */}
            {schedules.map((schedule, idx) => (
              <div key={schedule.emailId} className="flex flex-col items-center relative z-10 flex-1">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-lg mb-3 shadow-lg transition-all ${
                  schedule.status === 'invalid' ? 'bg-red-500' :
                  schedule.status === 'warning' ? 'bg-yellow-500' : 
                  'bg-gradient-to-br from-blue-600 to-indigo-600'
                }`}>
                  {schedule.stepNumber}
                </div>
                <p className="text-sm font-semibold text-gray-900 mb-1">
                  {new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(schedule.scheduledTime)}
                </p>
                <p className="text-xs text-gray-500">
                  {new Intl.DateTimeFormat('en-US', { hour: 'numeric', minute: '2-digit' }).format(schedule.scheduledTime)}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <h4 className="text-lg font-bold text-red-900 mb-2">
                {schedules.filter(s => s.status === 'invalid').length} issue(s) need attention
              </h4>
              <ul className="space-y-1.5">
                {schedules.filter(s => s.status === 'invalid').map(schedule =>
                  schedule.errors.map((error, idx) => (
                    <li key={`${schedule.emailId}-${idx}`} className="text-sm text-red-700 flex items-start gap-2">
                      <span className="font-bold">•</span>
                      <span><strong>Email {schedule.stepNumber}:</strong> {error.message}</span>
                    </li>
                  ))
                )}
              </ul>
